/**
 * Roaring.io PEP (Politically Exposed Person) API Route
 *
 * GET /api/roaring/pep?ssn=YYYYMMDDXXXX&country=SE
 *
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Check PEP status
    const result = await roaringClient.checkPEP(ssn, country);
    ...
    console.error('Roaring.io PEP API Error:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check PEP status
    const result = await roaringClient.checkPEP(ssn, country.toUpperCase());
