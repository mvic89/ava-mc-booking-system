...
export async function PUT(
  ...
  try {
    ...
    // Sync trade-ins to dedicated table whenever the tradeIns/tradeInData fields were touched
    if ('tradeIns' in fields || 'tradeInData' in fields) {
      const tradeInsArr = (() => {
        const arr = fields.tradeIns;
      ...
      await syncTradeIns(Number(id), Number(offer.leadId), dealershipId as string, tradeInsArr);
    }
