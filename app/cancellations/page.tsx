...
export default function CancellationsPage() {
  ...
  const reasonLabels: Record<string, string> = {
    changed_mind:     t('reasons.changed_mind'),
    financial:        t('reasons.financial'),
    found_elsewhere:  t('reasons.found_elsewhere'),
    financing_denied: t('reasons.financing_denied'),
    other:            t('reasons.other'),
  };

  const refundStatusLabels: Record<string, string> = {
    pending:   t('refundStatus.pending'),
    sent:      t('refundStatus.sent'),
    confirmed: t('refundStatus.confirmed'),
  };
  ...
  const kpis = [
    { label: t('kpi.total'),     value: cancellations.length,                                         icon: '❌', alert: false },
    { label: t('kpi.refunds'),   value: fmtKr(totalRefunds),                                          icon: '💸', alert: false },
    { label: t('kpi.pending'),   value: pendingRefunds,                                               icon: '⏳', alert: pendingRefunds > 0 },
    { label: t('kpi.topReason'), value: topReason ? reasonLabels[topReason[0]] ?? topReason[0] : '—', icon: '📊', alert: false },
  ];
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100">
                    {[t('table.date'), t('table.customer'), t('table.vehicle'), t('table.reason'), t('table.refund'), t('table.refundStatus'), t('table.by'), ''].map((h, i) => (
                  ...
                  {cancellations.map(c => (
                    <tr key={c.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div>
                          <span className="text-slate-700 font-medium">{reasonLabels[c.reason] ?? c.reason}</span>
                      ...
                      <td className="px-4 py-3">
                        {c.refund_amount > 0 ? (
                          <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${REFUND_STATUS_BADGE[c.refund_status] ?? 'bg-slate-100 text-slate-600'}`}>
                            {refundStatusLabels[c.refund_status] ?? c.refund_status}
                          </span>
